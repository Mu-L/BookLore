...
public class PdfMetadataWriter implements MetadataWriter {
    ...
    @Override
    public void writeMetadataToFile(File file, BookMetadataEntity metadataEntity, String thumbnailUrl) {
        ...
        try (PDDocument pdf = Loader.loadPDF(file)) {
            ...
            applyMetadataToDocument(pdf, metadataEntity);
    ...
    private void applyMetadataToDocument(PDDocument pdf, BookMetadataEntity entity) {
        PDDocumentInformation info = pdf.getDocumentInformation();
        MetadataCopyHelper helper = new MetadataCopyHelper(entity);

        helper.copyTitle(info::setTitle);
        helper.copyPublisher(info::setProducer);
        helper.copyAuthors(authors -> info.setAuthor(String.join(", ", authors)));
        helper.copyCategories(cats -> info.setKeywords(String.join(", ", cats)));

        try {
            XMPMetadata xmp = XMPMetadata.createXMPMetadata();
            DublinCoreSchema dc = xmp.createAndAddDublinCoreSchema();

            helper.copyTitle(dc::setTitle);
            helper.copyDescription(dc::setDescription);
            helper.copyPublisher(dc::addPublisher);
            helper.copyLanguage(dc::addLanguage);
            helper.copyPublishedDate(date -> {
                Calendar cal = GregorianCalendar.from(date.atStartOfDay(ZoneId.systemDefault()));
                dc.addDate(cal);
            });
            helper.copyAuthors(authors -> authors.forEach(dc::addCreator));
            helper.copyCategories(categories -> categories.forEach(dc::addSubject));

            ByteArrayOutputStream baos = new ByteArrayOutputStream();
            new XmpSerializer().serialize(xmp, baos, true);
            byte[] baseXmpBytes = baos.toByteArray();
            byte[] newXmpBytes = addCustomIdentifiersToXmp(baseXmpBytes, entity, helper);
    ...
    private byte[] addCustomIdentifiersToXmp(byte[] xmpBytes, BookMetadataEntity metadata, MetadataCopyHelper helper) throws Exception {
        ...
        helper.copyGoogleId(id -> appendIdentifier(doc, rdfBag, "google", id));
        helper.copyGoodreadsId(id -> appendIdentifier(doc, rdfBag, "goodreads", id));
        helper.copyHardcoverId(id -> appendIdentifier(doc, rdfBag, "hardcover", id));
        helper.copyAsin(id -> appendIdentifier(doc, rdfBag, "amazon", id));
        helper.copyIsbn13(id -> appendIdentifier(doc, rdfBag, "isbn", id));
        ...
        helper.copyPersonalRating(rating -> {
            calibreDescription.appendChild(createSimpleElement(doc, "calibre:rating", String.valueOf(rating.intValue())));
        });

        helper.copySeriesName(series -> {
